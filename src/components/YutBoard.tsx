...
const YutBoard = ({ pieces, teams, onMovePiece, currentTurn, logic, svgRef }: YutBoardProps) => {
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent, pieceId: string) => {
    ...
    dragStartRef.current = { x: pos.x, y: pos.y, startTime: Date.now() }; // 시작 위치 & 시간 저장
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    // 드래그 중에는 선택된 말 ID를 해제하지 않음 (드래그 종료 시 판별)
  }, [drag, clientToSVG, setDrag]);
  ...
  const handlePointerUp = useCallback((e: React.PointerEvent) => {
    ...
    const dist = Math.sqrt(Math.pow(drag.currentX - startPos.x, 2) + Math.pow(drag.currentY - startPos.y, 2));
    const duration = Date.now() - startPos.startTime;

    // 클릭으로 간주 (500ms 미만, 15px 미만 이동 - 모바일 터치 흔들림 보정)
    if (dist < 15 && duration < 500) {
      setSelectedPieceId(prev => prev === drag.pieceId ? null : drag.pieceId);
    } else {
       // 드래그 이동 처리
      if (nearest) {
  ...
  }, [drag, onMovePiece, pieces, setDrag, setSelectedPieceId]);
  ...
  return (
    <div className="relative w-full max-w-[1000px] mx-auto touch-none select-none">
      <svg
        ref={svgRef}
        viewBox={`0 0 600 600`}
      ...
      {/* 이동 툴팁 메뉴 - HTML Overlay (SVG 밖에서 렌더링) */}
